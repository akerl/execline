...
static int myexlp (stralloc *sa, char const *const *argv, unsigned int argc, unsigned int nmin, char const *dollar0)
{
  ...
  if (!genalloc_ready(elsubst, &info.data, 3 + n)) return -1 ;
  ...
  stralloc_catb(&info.vars, "#\0" "0\0@", 6) ;
  {
    ...
    if (!stralloc_cats(&info.values, dollar0) || !stralloc_0(&info.values)) goto err ;
    blah[2].var = 4 ; blah[2].value = info.values.len ; blah[2].n = argc ;
    genalloc_catb(elsubst, &info.data, blah, 3) ;
  }
  for (; i < n ; i++)
  {
    elsubst blah ;
    ...
    if (!stralloc_cats(&info.values, i < argc ? argv[i] : "") || !stralloc_0(&info.values)) goto err ;
    genalloc_append(elsubst, &info.data, &blah) ;
  }
  {
    stralloc dst = STRALLOC_ZERO ;
    int r = el_substitute(&dst, sa->s, sa->len, info.vars.s, info.values.s, genalloc_s(elsubst, &info.data), genalloc_len(elsubst, &info.data)) ;
    if (r < 0) goto err ;
    exlsn_free(&info) ;
    stralloc_free(sa) ;
    *sa = dst ;
