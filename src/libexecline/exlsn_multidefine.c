...
int exlsn_multidefine (int argc, char const **argv, char const *const *envp, exlsn_t_ref info)
{
  ...
  for (; i < (unsigned int)argc1 ; i++)
  {
    if (*argv[i])
    {
      ...
      if (!stralloc_cats(&info->vars, argv[i]) || !stralloc_0(&info->vars)) goto err ;
  ...
  if ((i < max) && likeread) genalloc_s(elsubst, &info->data)[i-1].n = max - i + 1 ;
